...
# 'leading'을 'preceding'으로 이름 변경
merged_data.rename(columns={'leading': 'preceding', 'leading_rel_v_x': 'preceding_rel_v_x', 
                            'leading_rel_v_y': 'preceding_rel_v_y', 'leading_distance': 'preceding_distance'}, inplace=True)
...
# 상대 속도 계산 및 새로운 열 추가
surrounding_columns = ['preceding', 'right_leading', 'left_leading', 'following', 'right_following', 'left_following']
...
for index, row in merged_data.iterrows():
    ...
    for col in surrounding_columns:
        surr_vehicle_id = row[col]
        if not pd.isna(surr_vehicle_id):
            if col in ['preceding', 'right_leading', 'left_leading']:
...
final_columns = ['frame', 'ID', 'center_x', 'center_y', 'v_x', 'v_y', 'a_x', 'a_y', 'lane', 
                 'preceding_rel_v_x', 'preceding_rel_v_y', 'right_leading_rel_v_x', 'right_leading_rel_v_y',
                 'left_leading_rel_v_x', 'left_leading_rel_v_y', 'following_rel_v_x', 'following_rel_v_y',
                 'right_following_rel_v_x', 'right_following_rel_v_y', 'left_following_rel_v_x', 'left_following_rel_v_y',
                 'preceding_distance', 'right_leading_distance', 'left_leading_distance', 
