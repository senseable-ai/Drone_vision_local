...
def find_neighboring_ids(frame_data, target_id, roi):
    ...
    for _, row in frame_data.iterrows():
        ...
        if center_y < center_y_target:  # 차량이 앞쪽에 존재
            if center_x < center_x_target:  # 차량이 좌측에 존재
                preceding_left_id = row['ID']
            elif center_x > center_x_target:  # 차량이 우측에 존재
                preceding_right_id = row['ID']
            else:
                preceding_id = row['ID']
        elif center_y > center_y_target:  # 차량이 후방에 존재
            if center_x < center_x_target:  # 차량이 좌측에 존재
